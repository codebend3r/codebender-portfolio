...
- The Angular route, PDF, and DOCX exports are unaffected.
...
1. **Streaming synchronous Netlify function, full resume in the system prompt
   (recommended).** The resume is ~27k characters (~7k tokens), far inside the
...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/utils/period.ts`                  | Extract the existing `PERIOD_PATTERN` parsing out of `experienceDuration.ts` into `parsePeriod({ period, now })` returning `{ start, end }` as absolute month indexes, or `null`. `experienceDuration` is rewritten on top of it with no behavior change (its tests stay green).                                                                                           |
...
- **Caching:** the system prefix is ~8k tokens, well above Claude Opus 5's
...
figure built from the real dates.
...
- The Angular route, PDF, and DOCX exports.
